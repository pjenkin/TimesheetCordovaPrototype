...
//      bound parameters not suitable for    SELECT statements ??  http://php.net/manual/en/mysqli-stmt.execute.php
     ...
     //code below is suitable for possible many rig/project/worktype to 1 shift
     
    $q2=mysqli_query($connection,"SELECT DISTINCT tblShift.ShiftID, tblProject.ProjectDescription, tblProject.ProjectID, tblProject.ProjectDescription FROM tblOperatorShift INNER JOIN (tblShift INNER JOIN (tblProjectShift INNER JOIN tblProject ON tblProject.ProjectID = tblProjectShift.ProjectID) ON tblShift.ShiftID = tblProjectShift.ShiftID) ON tblOperatorShift.ShiftID = tblShift.ShiftID WHERE tblOperatorShift.OperatorID=$operatorID");
     // not so simple! need to tie this to tblOperator for every result in Shift query (every ShiftID)
     // make array of ShiftIDs to append to string?
    $q3=mysqli_query($connection,"SELECT DISTINCT tblShift.ShiftID, tblRig.RigDescription, tblRig.RigID FROM tblOperatorShift INNER JOIN (tblShift INNER JOIN (tblRigShift INNER JOIN tblRig ON tblRig.RigID = tblRigShift.RigID) ON tblShift.ShiftID = tblRigShift.ShiftID) ON tblOperatorShift.ShiftID = tblShift.ShiftID WHERE tblOperatorShift.OperatorID=$operatorID");
     
    $q4=mysqli_query($connection,"SELECT DISTINCT tblShift.ShiftID, tblWorkType.WorkTypeDescription, tblWorkType.WorkTypeID FROM tblOperatorShift INNER JOIN (tblShift INNER JOIN (tblWorkTypeShift INNER JOIN tblWorkType ON tblWorkType.WorkTypeID = tblWorkTypeShift.WorkTypeID) ON tblShift.ShiftID = tblWorkTypeShift.ShiftID) ON tblOperatorShift.ShiftID = tblShift.ShiftID WHERE tblOperatorShift.OperatorID=$operatorID");
     
     
     // these might be better with single line queries?? but can't be sure that only 1 rig or worktype per shift?
     // query below is only good if there is, per shift, only 1 project, 1 rig, 1 work type (1 operator, of course)

     // SELECT DISTINCT tblShift.ShiftID, tblProject.ProjectDescription, tblProject.ProjectID, tblProject.ProjectDescription, tblRig.RigDescription, tblRig.RigID, tblWorkType.WorkTypeDescription, tblWorkType.WorkTypeID, tblShift.StartDateTime, tblShift.EndDateTime, tblShift.WorkHours, tblShift.Product, tblOperator.OperatorID, tblOperator.OperatorName FROM tblOperator INNER JOIN (tblWorkType INNER JOIN ( tblWorkTypeShift INNER JOIN ( tblRig INNER JOIN ( tblRigShift INNER JOIN ( tblOperatorShift INNER JOIN (tblShift INNER JOIN (tblProjectShift INNER JOIN tblProject ON tblProject.ProjectID = tblProjectShift.ProjectID) ON tblShift.ShiftID = tblProjectShift.ShiftID) ON tblOperatorShift.ShiftID = tblShift.ShiftID) ON tblRigShift.ShiftID = tblOperatorShift.ShiftID) ON tblRig.RigID = tblRigShift.RigID) ON tblWorkTypeShift.ShiftID = tblOperatorShift.ShiftID) ON tblWorkType.WorkTypeID = tblWorkTypeShift.WorkTypeID) ON tblOperatorShift.OperatorID = tblOperator.OperatorID WHERE tblOperatorShift.OperatorID=2
     
     // tblShift.ShiftID, tblProject.ProjectDescription, tblProject.ProjectID, tblProject.ProjectDescription, tblRig.RigDescription, tblRig.RigID, tblWorkType.WorkTypeDescription, tblWorkType.WorkTypeID, tblShift.StartDateTime, tblShift.EndDateTime, tblShift.WorkHours, tblShift.Product
     
     
      $q1=mysqli_query($connection,"SELECT DISTINCT tblShift.ShiftID, tblProject.ProjectDescription, tblProject.ProjectID, tblProject.ProjectDescription, tblRig.RigDescription, tblRig.RigID, tblWorkType.WorkTypeDescription, tblWorkType.WorkTypeID, tblShift.StartDateTime, tblShift.EndDateTime, tblShift.WorkHours, tblShift.Product, tblOperator.OperatorID, tblOperator.OperatorName FROM tblOperator INNER JOIN (tblWorkType INNER JOIN ( tblWorkTypeShift INNER JOIN ( tblRig INNER JOIN ( tblRigShift INNER JOIN ( tblOperatorShift INNER JOIN (tblShift INNER JOIN (tblProjectShift INNER JOIN tblProject ON tblProject.ProjectID = tblProjectShift.ProjectID) ON tblShift.ShiftID = tblProjectShift.ShiftID) ON tblOperatorShift.ShiftID = tblShift.ShiftID) ON tblRigShift.ShiftID = tblOperatorShift.ShiftID) ON tblRig.RigID = tblRigShift.RigID) ON tblWorkTypeShift.ShiftID = tblOperatorShift.ShiftID) ON tblWorkType.WorkTypeID = tblWorkTypeShift.WorkTypeID) ON tblOperatorShift.OperatorID = tblOperator.OperatorID WHERE tblOperatorShift.ShiftID=$shiftID");
     
     //NB the below will work only if 1:1 shift:project:operator:rig:worktype - if many:1 (e.g. 1 operator using several rigs during a shift) this would need to change (maybe back to nested JSON records within fields)
     
      $q1=mysqli_query($connection,"SELECT DISTINCT tblShift.ShiftID, tblProject.ProjectDescription, tblProject.ProjectID, tblProject.ProjectDescription, tblRig.RigDescription, tblRig.RigID, tblWorkType.WorkTypeDescription, tblWorkType.WorkTypeID, tblShift.StartDateTime, tblShift.EndDateTime, tblShift.WorkHours, tblShift.Product, tblShift.Date, tblOperator.OperatorID, tblOperator.OperatorName, tblOperatorShift.OperatorShiftID, tblProjectShift.ProjectShiftID, tblRigShift.RigShiftID, tblWorkTypeShift.WorkTypeShiftID FROM tblOperator INNER JOIN (tblWorkType INNER JOIN ( tblWorkTypeShift INNER JOIN ( tblRig INNER JOIN ( tblRigShift INNER JOIN ( tblOperatorShift INNER JOIN (tblShift INNER JOIN (tblProjectShift INNER JOIN tblProject ON tblProject.ProjectID = tblProjectShift.ProjectID) ON tblShift.ShiftID = tblProjectShift.ShiftID) ON tblOperatorShift.ShiftID = tblShift.ShiftID) ON tblRigShift.ShiftID = tblOperatorShift.ShiftID) ON tblRig.RigID = tblRigShift.RigID) ON tblWorkTypeShift.ShiftID = tblOperatorShift.ShiftID) ON tblWorkType.WorkTypeID = tblWorkTypeShift.WorkTypeID) ON tblOperatorShift.OperatorID = tblOperator.OperatorID WHERE tblOperatorShift.ShiftID=$shiftID");
...
// multi-query (nested json)  for many project/rig/worktype : 1 shift REMmed below         
/*     
