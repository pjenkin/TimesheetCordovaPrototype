...
//alert("parsedShift=" + parsedShift);
                    $('body').html(new ShiftView(parsedShift).render().$el);        // inject (?) the view's HTML using the handlebars template and with parsed Shift data as argument   
                    
                                
                    
                              $('#startdatetimeinput').click(function(){   // set up newly added startdatetimeinput element's click
...
//alert(myDate);                                            
                              ...
                              $('#enddatetimeinput').click(function(){   // set up newly added enddatetimeinput element's click
...
//alert(myDate);                                            
                              ...
                              $('#dateinput').click(function(){   // set up newly added element's click

// https://www.npmjs.com/package/skwas-cordova-plugin-datetimepicker
                                    var myDate = new Date($('#dateinput').val()); // get date from form input 
//alert("enddatetime" + myDate); 
// NB if time ever at 0000-00-00 00:00:00 then the plugin dialogue won't work (field value must be set to non-zero at database)                                  
                                    cordova.plugins.DateTimePicker.show({
                                        mode: "date",
                                        ...
                                        success: function(newDate) {
                                            ...
                                            console.info(newDate);
                                            myDate = newDate;
                                            $('#dateinput').val(myDate);       // set form input's date value from user action on datetime picker
//alert(myDate);                                            
                                        },
                                        cancel: function() {
                                            console.info("Cancelled");
                                ...
                                });     // end of setting up date field box click
...
//alert(response + ", status:" + status);
            ...
            router.addRoute('newshiftview', function () {
// apparently it was necessary for the anchor here to be without a slash
                    $('body').html(new NewShiftView().render().$el);        // inject (?) the view's HTML using the handlebars template   
                    
                    d = new Date(); // get the present date, to use as starter suggested value for new date/time fields (wonky if zeros)
                    
                // not sure if zero-padding digits actually necessary for MySQL yyyy-mm-dd
                    $('#dateinput').val(d.getFullYear() + "-" + (d.getMonth() < 10 ? '0' : '') + d.getMonth()  + "-" + (d.getDate() < 10 ? '0' : '') + d.getDate());
                    $('#startdatetimeinput').val(d.getFullYear() + "-" + d.getMonth() + "-" + d.getDate() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ".0");
                    $('#enddatetimeinput').val(d.getFullYear() + "-" + d.getMonth() + "-" +  d.getDate() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ".0");
...
//alert(myDate);                                            
                                ...
                                });     // end of setting up date click
                
                
                    
                    $('#btn-newshiftinsert').click(function(){   // set up newly added element's click (button to save chages to shift's record)
                            // $('#frm-shiftupdate').submit();      // trigger the shift's form's submit action (a post to updateallshifttables.php)
                        // https://www.w3schools.com/jquery/ajax_post.asp
//alert('in btn-newshiftinsert function');                        
//alert();
                                $.post("http://www.guidebookplus.com/drilldowndb/insertallnewshifttables.php",
                        
                                    {
                                        operatorid : $('#operatoridselect').val(),
                                        projectid : $('#projectidselect').val(),
                                        rigid : $('#rigidselect').val(),
                                        worktypeid : $('#worktypeidselect').val(),
                                        product : $('#productinput').val(),
